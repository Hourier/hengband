 ...
 * @param dungeon_level 生成基準階
 * @return 生成許可ならばtrue、禁止ならばfalse
 * @details クエストモンスター、ダンジョンの主、指定階未満でのFORCE_DEPTH フラグ持ちは生成禁止
 */
bool MonraceAllocationEntry::is_permitted(int dungeon_level) const
{
    ...
    if (monrace.misc_flags.has(MonsterMiscType::FORCE_DEPTH) && (monrace.level > dungeon_level)) {
        return false;
    }
 ...
 */
bool MonraceAllocationEntry::is_defeatable(int dungeon_level) const
{
    const auto &monrace = this->get_monrace();
    ...
    const auto is_shallow = monrace.level > dungeon_level;
