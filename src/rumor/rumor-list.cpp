...
void RumorList::add_towns()
{
    ...
    for (auto i = 0; i < std::ssize(towns_info); ++i) {
        if (i != SECRET_TOWN) {
            rumors.push_back(str_replace(this->type_template.at(RumorType::TOWN), name_template, towns_info[i].name));
        }
    }
}
...
void RumorList::add_shallow_dungeons()
{
    ...
    for (const auto &[_, dungeon] : DungeonList::get_instance()) {
        if (0 < dungeon->mindepth && dungeon->mindepth <= depth_threshold) {
            rumors.push_back(str_replace(this->type_template.at(RumorType::SHALLOW_DUNGEON), name_template, dungeon->name));
        }
    }
}
...
void RumorList::add_normal_monsters()
{
    ...
    for (const auto &name : MonraceList::get_instance().get_normal_monster_names()) {
        rumors.push_back(str_replace(this->type_template.at(RumorType::NORMAL_MONSTER), name_template, name.string()));
    }
}
...
void RumorList::add_shallow_artifacts()
{
    ...
    for (const auto &[id, artifact] : ArtifactList::get_instance()) {
        ...
        if (artifact.level <= depth_threshold) {
            rumors.push_back(str_replace(this->type_template.at(RumorType::SHALLOW_ARTIFACT), name_template, artifact.get_full_name()));
        }
    }
}
...
void RumorList::add_deep_dungeons()
{
    ...
    for (const auto &[_, dungeon] : DungeonList::get_instance()) {
        if (dungeon->mindepth > depth_threshold) {
            rumors.push_back(str_replace(this->type_template.at(RumorType::DEEP_DUNGEON), name_template, dungeon->name));
        }
    }
}
...
void RumorList::add_unique_monsters()
{
    ...
    for (const auto &name : MonraceList::get_instance().get_unique_monster_names()) {
        rumors.push_back(str_replace(this->type_template.at(RumorType::UNIQUE_MONSTER), name_template, name.string()));
    }
}
...
void RumorList::add_deep_artifacts()
{
    ...
    for (const auto &[id, artifact] : ArtifactList::get_instance()) {
        if (artifact.level > depth_threshold) {
            rumors.push_back(str_replace(this->type_template.at(RumorType::DEEP_ARTIFACT), name_template, artifact.get_full_name()));
        }
    }
}
