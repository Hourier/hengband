...
parse_error_type parse_line_feature(const FloorType &floor, std::string_view buf)
{
    ...
    const auto tokens = tokenize(buf.substr(2), 9, false);
    ...
    case 2: {
        ...
        if (token.starts_with('*') && token.length() == 1) {
...
parse_error_type parse_line_building(std::string_view buf)
{
    ...
    switch (s[0]) {
    case 'N': {
        const auto tokens = tokenize(s.substr(2), 3, false);
        if (tokens.size() == 3) {
    ...
    case 'A': {
        const auto tokens = tokenize(s.substr(2), 7, false);
        if (tokens.size() >= 7) {
    ...
    case 'C': {
        const auto tokens = tokenize(s.substr(2), PLAYER_CLASS_TYPE_MAX, false);
    ...
    case 'R': {
        const auto tokens = tokenize(s.substr(2), MAX_RACES, false);
    ...
    case 'M': {
        const auto tokens = tokenize(s.substr(2), MAX_MAGIC, false);
