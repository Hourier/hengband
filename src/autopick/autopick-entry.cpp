...
public:
    ParsedNumericValue(int v, std::string_view sv)
        : value(v)
        , remaining_sv(sv)
    {
    }
...
{
    std::string_view current_sv = ltrim_sv(sv);
    auto k = 0;
    auto parsed_value = 0;
...
bool autopick_new_entry(autopick_type *entry, std::string_view str_view, bool allow_default)
{
    ...
    do {
        ...
        old_sv = sv;
        static const std::vector<std::pair<std::string_view, int>> general_flags = {
            { KEY_ALL, FLG_ALL },
        ...
        for (const auto &[key, flag] : general_flags) {
        ...
        static const std::vector<std::pair<std::string_view, int>> other_flags = {
        ...
        for (const auto &[key, flag] : other_flags) {
            if (const auto sv_opt = match_key(sv, key); sv_opt) {
                entry->add(flag);
                sv = *sv_opt;
                if (sv.empty()) {
    ...
    static const std::vector<std::pair<std::string_view, int>> item_type_flags = {
        { KEY_ITEMS, FLG_ITEMS },
    ...
    for (const auto &[key, flag] : item_type_flags) {
        if (const auto sv_opt = match_key(sv, key); sv_opt) {
            entry->add(flag);
            previous_flag = flag;
            sv = *sv_opt;
