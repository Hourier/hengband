...
static void describe_autpick_jp(char *buff, const autopick_type &entry, autopick_describer *describer)
{
    ...
    if (entry.has(FLG_COLLECTING)) {
        before_strings.push_back("収集中で既に持っているスロットにまとめられる");
    }

    if (entry.has(FLG_UNAWARE)) {
        before_strings.push_back("未鑑定でその効果も判明していない");
    }

    if (entry.has(FLG_UNIDENTIFIED)) {
        before_strings.push_back("未鑑定の");
    }

    if (entry.has(FLG_IDENTIFIED)) {
        before_strings.push_back("鑑定済みの");
    }

    if (entry.has(FLG_STAR_IDENTIFIED)) {
        before_strings.push_back("完全に鑑定済みの");
    }

    if (entry.has(FLG_BOOSTED)) {
        before_strings.push_back("ダメージダイスが通常より大きい");
        describer->body_str = "武器";
    }

    if (entry.has(FLG_MORE_DICE)) {
        static char more_than_desc_str[] = "___";
        before_strings.push_back("ダメージダイスの最大値が");
        describer->body_str = "武器";

        snprintf(more_than_desc_str, sizeof(more_than_desc_str), "%d", entry.dice);
        before_strings.push_back(more_than_desc_str);
        before_strings.push_back("以上の");
    }

    if (entry.has(FLG_MORE_BONUS)) {
        static char more_bonus_desc_str[] = "___";
        before_strings.push_back("修正値が(+");

        snprintf(more_bonus_desc_str, sizeof(more_bonus_desc_str), "%d", entry.bonus);
        before_strings.push_back(more_bonus_desc_str);
        before_strings.push_back(")以上の");
    }

    if (entry.has(FLG_WORTHLESS)) {
        before_strings.push_back("店で無価値と判定される");
    }

    if (entry.has(FLG_ARTIFACT)) {
        before_strings.push_back("アーティファクトの");
        describer->body_str = "装備";
    }

    if (entry.has(FLG_EGO)) {
        before_strings.push_back("エゴアイテムの");
        describer->body_str = "装備";
    }

    if (entry.has(FLG_GOOD)) {
        before_strings.push_back("上質の");
        describer->body_str = "装備";
    }

    if (entry.has(FLG_NAMELESS)) {
        before_strings.push_back("エゴでもアーティファクトでもない");
        describer->body_str = "装備";
    }

    if (entry.has(FLG_AVERAGE)) {
        before_strings.push_back("並の");
        describer->body_str = "装備";
    }

    if (entry.has(FLG_RARE)) {
        before_strings.push_back("ドラゴン装備やカオス・ブレード等を含む珍しい");
        describer->body_str = "装備";
    }

    if (entry.has(FLG_COMMON)) {
        before_strings.push_back("ありふれた(ドラゴン装備やカオス・ブレード等の珍しい物ではない)");
        describer->body_str = "装備";
    }

    if (entry.has(FLG_WANTED)) {
        before_strings.push_back("ハンター事務所で賞金首とされている");
        describer->body_str = "死体や骨";
    }

    if (entry.has(FLG_HUMAN)) {
        before_strings.push_back("悪魔魔法で使うための人間やヒューマノイドの");
        describer->body_str = "死体や骨";
    }

    if (entry.has(FLG_UNIQUE)) {
        before_strings.push_back("ユニークモンスターの");
        describer->body_str = "死体や骨";
    }

    if (entry.has(FLG_UNREADABLE)) {
        before_strings.push_back("あなたが読めない領域の");
        describer->body_str = "魔法書";
    }

    if (entry.has(FLG_REALM1)) {
        before_strings.push_back("第一領域の");
        describer->body_str = "魔法書";
    }

    if (entry.has(FLG_REALM2)) {
        before_strings.push_back("第二領域の");
        describer->body_str = "魔法書";
    }

    if (entry.has(FLG_FIRST)) {
        before_strings.push_back("全4冊の内の1冊目の");
        describer->body_str = "魔法書";
    }

    if (entry.has(FLG_SECOND)) {
        before_strings.push_back("全4冊の内の2冊目の");
        describer->body_str = "魔法書";
    }

    if (entry.has(FLG_THIRD)) {
        before_strings.push_back("全4冊の内の3冊目の");
        describer->body_str = "魔法書";
    }

    if (entry.has(FLG_FOURTH)) {
        before_strings.push_back("全4冊の内の4冊目の");
        describer->body_str = "魔法書";
    }
...
void describe_autopick_en(char *buff, const autopick_type &entry, autopick_describer *describer)
{
    ...
    if (entry.has(FLG_UNAWARE)) {
        before_strings.push_back("unidentified");
    ...
    if (entry.has(FLG_IDENTIFIED)) {
        before_strings.push_back("identified");
    }

    if (entry.has(FLG_STAR_IDENTIFIED)) {
        before_strings.push_back("fully identified");
    }

    if (entry.has(FLG_RARE)) {
        before_strings.push_back("very rare");
        after_str[after_n++] = "such as Dragon armor, Blades of Chaos, etc.";
    }

    if (entry.has(FLG_COMMON)) {
        before_strings.push_back("relatively common");
        after_str[after_n++] = "compared to very rare Dragon armor, Blades of Chaos, etc.";
    }

    if (entry.has(FLG_WORTHLESS)) {
        before_strings.push_back("worthless");
        which_str[which_n++] = "can not be sold at stores";
    }

    if (entry.has(FLG_ARTIFACT)) {
        before_strings.push_back("artifact");
    }

    if (entry.has(FLG_EGO)) {
        before_strings.push_back("ego");
    }
    ...
    if (entry.has(FLG_HUMAN)) {
        before_strings.push_back("humanoid");
        describer->body_str = "corpses or skeletons";
        which_str[which_n++] = "can be used for Daemon magic";
    }

    if (entry.has(FLG_UNIQUE)) {
        before_strings.push_back("unique monsters'");
        describer->body_str = "corpses or skeletons";
    }
    ...
    if (entry.has(FLG_FIRST)) {
        before_strings.push_back("first one of four");
        describer->body_str = "spellbooks";
    }

    if (entry.has(FLG_SECOND)) {
        before_strings.push_back("second one of four");
        describer->body_str = "spellbooks";
    }

    if (entry.has(FLG_THIRD)) {
        before_strings.push_back("third one of four");
        describer->body_str = "spellbooks";
    }

    if (entry.has(FLG_FOURTH)) {
        before_strings.push_back("fourth one of four");
        describer->body_str = "spellbooks";
    }
