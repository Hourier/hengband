...
{
    const auto tokens = tokenize(buf.substr(2), 3, true);
    if (tokens.size() != 3) {
        return false;
    }
...
{
    const auto tokens = tokenize(buf.substr(2), F_LIT_MAX * 2 + 1, true);
...
static bool interpret_s_token(std::string_view buf)
{
    ...
    const auto color = static_cast<uint8_t>(std::stoi(tokens[1], nullptr, 0));
    const auto character = static_cast<char>(std::stoi(tokens[2], nullptr, 0));
    misc_to_attr[num] = color;
    misc_to_char[num] = character;
    return true;
}
...
{
    const auto tokens = tokenize(buf.substr(2), 2, true);
    if (tokens.size() != 2) {
        return false;
    }
...
{
    const auto tokens = tokenize(buf.substr(2), 5, true);
    if (tokens.size() != 5) {
        return false;
    }
...
static void interpret_xy_token(PlayerType *player_ptr, std::string_view buf)
{
    ...
    for (auto &option : option_info) {
        ...
        if ((player_ptr->playing || world.character_xtra) && (GameOptionPage::BIRTH == option.page) && !world.wizard) {
            msg_print(fmt::format(_("初期オプションは変更できません! '{}'", "Birth options can not be changed! '{}'"), buf));
            msg_erase();
            return;
        }
...
{
    const auto tokens = tokenize(buf.substr(2), 2 + MAX_MACRO_MOD, false);
    ...
    return interpret_macro_keycodes(tokens.size(), tokens);
}
